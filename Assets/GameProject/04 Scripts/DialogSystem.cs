...
public class DialogSystem : MonoBehaviour
{
	...
	{
		int index = 0;
		for (int i = 0; i < rtsDB.DialogDB.Count; ++i)
        {
			if(rtsDB.DialogDB[i].branch == branch)
            {
				dialogs[index].dialogue = rtsDB.DialogDB[i].dialog;
				index++;
            }
        }
		Setup();
	...
	{
		speaker.dialogPanel.SetActive(visible);
